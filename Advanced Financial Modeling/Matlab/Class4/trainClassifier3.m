function [trainedClassifier,...
    validationAccuracy_nv,...
    validationAccuracy_v,...
    cm_nv,...
    cm_v,...
    predictors,...
    response] = trainClassifier3(trainingData)
...
% Auto-generated by MATLAB on 11-Sep-2019 00:39:30
...
response = inputTable.column_7;
isCategoricalPredictor = [false, false, false, false, false, false];
% results without cross_validation
[label,score] = predict(trainedClassifier.ClassificationSVM,predictors);
cm_nv=confusionmat(response,label);
validationAccuracy_nv=1-resubLoss(trainedClassifier.ClassificationSVM);
...
validationAccuracy_v = 1 - kfoldLoss(partitionedModel, 'LossFun', 'ClassifError');
cm_v=confusionmat(response,validationPredictions);
